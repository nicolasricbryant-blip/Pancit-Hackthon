...
- ~~Profile stat tiles: no per-player Scrims/Wins/WinRate source~~ — **DONE.** `20260910010014_player_match_stats.sql` adds a `public.player_match_stats` view (per profile_id + game_id: `scrims`/`wins`/`losses`/`win_rate_pct`), derived from confirmed `scrim_matches` + `bracket_matches` joined against **current** `team_members` (the schema has no per-match roster snapshot, so this is an approximation, not a historical record — documented in the view's comment). `/profile` now sums it across games and renders real numbers instead of `—`.
- ~~Player leaderboard: `ratings.rating` for profile_id rows never populated from real scrims~~ — **DONE.** `20260910010015_scrim_rating_mirror.sql` extends `apply_match_result()` (the confirmed-scrim trigger) with the same roster-mirror step `advance_bracket_match()` already did for brackets: on a confirmed match, the team's Elo rating is copied onto every current `team_members` row for that game. Scrims are the common case, so this is what actually lights up `/leaderboards?board=player` in practice. Same current-roster caveat as `player_match_stats` (documented in the migration).
- ~~Player board's Standing column always `—`~~ — **DONE.** `20260910010016_community_standing_mirror.sql` adds an `ensure_team_standing()` helper (a team outside the demo seed had no `community_standing` row, same gap `ensure_team_rating`/`ensure_team_reliability` already covered) and mirrors team standing onto current roster members the same way ratings are mirrored, in both `apply_match_result()` and `advance_bracket_match()`.
- ~~Desktop shell polish: wordmark sits alone above the rail; "Sign in" uses default styling~~ — **DONE.** `.app-header::after` now draws a divider at `var(--rail-w)` spanning the header height, so the wordmark cell reads as one brand column continuing into the rail below instead of floating alone. `ProfileMenu.module.css`'s `.signInLink` is now a filled `--color-primary` pill (matching the Request Scrim CTA's "important action is blue" language) instead of an outlined pill. Verified via a static rendered preview of the header markup/CSS at desktop width (no live Supabase auth available in this environment to run the real dev server signed-out).
- ~~Realtime for `/lobbies/[id]`~~ — **DONE.** `20260910010017_lobbies_realtime.sql` adds `lobbies` + `lobby_members` to the `supabase_realtime` publication; `LobbyActions.tsx` now subscribes to `postgres_changes` on both (scoped to the lobby) instead of polling `router.refresh()` every 10s.
- ~~`src/features/scrims/SkeletonCard.tsx` unused~~ — **DONE.** Added `src/app/loading.tsx` (Next's route-level Suspense fallback) rendering a `.feed-grid` of `SkeletonCard`s while `listScrimsForGame` resolves on `/`. The CSS (`.skeleton-card`/`.shimmer`/`.sk-*`) was already in `globals.css`, unused until now.
