...
- ~~Profile stat tiles: no per-player Scrims/Wins/WinRate source~~ — **DONE.** `20260910010014_player_match_stats.sql` adds a `public.player_match_stats` view (per profile_id + game_id: `scrims`/`wins`/`losses`/`win_rate_pct`), derived from confirmed `scrim_matches` + `bracket_matches` joined against **current** `team_members` (the schema has no per-match roster snapshot, so this is an approximation, not a historical record — documented in the view's comment). `/profile` now sums it across games and renders real numbers instead of `—`. Player leaderboard is untouched — it sorts by `ratings.rating`/`community_standing`, not by scrims/wins, so it's a separate gap (still needs per-player Elo to be computed from real matches, not just manually seeded).
